...
export default function ClientLogo({
  ...
  name: string;
  /** xs hoort bij een regel van 0.75rem, zoals op een ticketkaart. */
  size?: "xs" | "sm" | "md" | "lg";
}) {
  const dimensions =
    size === "lg" ? "w-16 h-16" : size === "md" ? "w-10 h-10" : size === "xs" ? "w-4 h-4" : "w-6 h-6";
  const textSize =
    size === "lg" ? "text-2xl" : size === "md" ? "text-lg" : size === "xs" ? "text-[9px]" : "text-xs";
